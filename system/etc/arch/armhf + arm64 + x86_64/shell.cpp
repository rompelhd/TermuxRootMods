...
std::vector<unsigned char> compute_hmac(const unsigned char* hmac_key,
    ...
    unsigned char result[HMAC_OUTLEN];
    unsigned int len;
...
bool encrypt_data(const std::string& plaintext,
    ...
    out.write((char*)iv, AES_IVLEN);
    out.write((char*)ciphertext.data(), ciphertext_len);
    out.write((char*)tag, GCM_TAGLEN);
    out.write((char*)hmac.data(), hmac.size());
    out.close();
...
bool decrypt_data(const std::string& password,
                  const std::string& salt,
    ...
    std::vector<unsigned char> data_to_hmac(
        file_data.begin(),
        file_data.begin() + file_data.size() - HMAC_OUTLEN);

    auto computed_hmac = compute_hmac(hmac_key, data_to_hmac);

    if (CRYPTO_memcmp(computed_hmac.data(), hmac, HMAC_OUTLEN) != 0)
        return false;
    ...
    EVP_DecryptInit_ex(ctx, nullptr, nullptr, aes_key, iv);
    ...
    plaintext_out.assign((char*)plaintext.data(), plaintext_len);
...
bool verify_connection(std::string& password, bool& password_prompted) {
    ...
    if (stat(timestamp_file.c_str(), &file_stat) == 0) {
        ...
        if (decrypt_data(stored_hash, salt, timestamp_file, decrypted_ts)) {
            ...
            if (current_time.tv_sec - timestamp < TIMEOUT_SECONDS) {
                debug_log("Timestamp valid. Access granted without password.");
                return true;
            }
        }
    ...
    std::string input_hash = derive_hash(password, salt);

    if (CRYPTO_memcmp(input_hash.c_str(),
                      stored_hash.c_str(),
                      stored_hash.size()) != 0)
        return false;
    ...
    encrypt_data(ts_str, stored_hash, salt, timestamp_file);
